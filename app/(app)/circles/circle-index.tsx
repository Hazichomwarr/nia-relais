...
function CircleCard({ circle }: { circle: OwnerCircleIndexItem }) {
  ...
  return (
    <li className="border-b border-[#e2d7c9] py-5 first:pt-0 last:border-b-0">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${STATUS_STYLES[circle.status]}`}>
            {circle.status}
          </span>
          <h2 className="mt-2 font-serif text-2xl tracking-tight text-[#173b32]">{circle.name}</h2>
        </div>
        <Link
          href={`/circles/${circle.id}`}
          className="inline-flex min-h-11 items-center rounded-full border border-[#35634f] px-4 text-sm font-semibold text-[#35634f] transition-colors hover:bg-[#dce9dc] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#b96549]"
        >
          Open circle
        </Link>
      </div>
      ...
      {round ? (
        <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm">
          ...
          <p className="text-xs text-[#587066]">Scheduled for {formatOwnerDate(round.dueDate)}</p>
        </div>
      ) : null}
    </li>
  );
    ...
    <main className="min-h-[calc(100vh-73px)] bg-[#fbf7ef] px-5 py-10 text-[#173b32] sm:px-8">
      <div className="mx-auto w-full max-w-6xl">
        <div className="flex flex-wrap items-end justify-between gap-5">
          ...
          <Link href="/circles/new" className="inline-flex min-h-11 items-center rounded-full bg-[#173b32] px-5 text-sm font-semibold text-white transition-colors hover:bg-[#285347] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#b96549]">
            + Create a circle
          </Link>
        </div>

        {circles.length === 0 ? (
          <section className="mt-8 border-y border-[#e2d7c9] py-10 text-center sm:py-14">
